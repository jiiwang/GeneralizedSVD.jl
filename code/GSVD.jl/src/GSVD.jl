...
include("csd.jl")
# export GSVD, gsvd, gsvd!

# Immutable composite type GSVD
struct GSVD{T,S} <: Factorization{T}
    ...
    # Inner constructor
    function GSVD{T,S}(U::AbstractMatrix{T}, V::AbstractMatrix{T},
                       Q::AbstractMatrix{T}, D1::AbstractMatrix{T},
                       D2::AbstractMatrix{T}, k::Int, l::Int,
                       R::AbstractMatrix{T}) where {T,S}
        new(U, V, Q, D1, D2, k, l, R)
    end

    # show() is used for neat display
    function show(io::IO, mime::MIME{Symbol("text/plain")},
                  F::GSVD{<:Any,<:AbstractArray})
...
# Outer constructor
function GSVD(U::AbstractMatrix{T}, V::AbstractMatrix{T},
              Q::AbstractMatrix{T}, D1::AbstractMatrix{T},
              D2::AbstractMatrix{T}, k::Int, l::Int,
              R::AbstractMatrix{T}) where T
    GSVD{T,typeof(U)}(U, V, Q, D1, D2, k, l, R)
end

"""
    gsvd(A, B) computes the generalized singular value decomposition (GSVD)
    of an m-by-n matrix A and p-by-n matrix B,

```math
     A = F.U * F.D1 * F.R * (F.Q)',    B = F.V * F.D2 * F.R * F.Q'
```

- U is a m-by-m orthogonal matrix,
- V is a p-by-p orthogonal matrix,
- Q is a n-by-n orthogonal matrix,
- R is a (k+l)-by-n matrix of structure [0 R1] where R1 is (k+l)-by-(k+l)
  and is nonsingular upper triangular matrix,
- k+l is the effective numerical rank of the matrix [A; B],
- D1 and D2 are m-by-(k+l) and p-by-(k+l) ``diagonal" matrices
  satisfying D1' * D1 + D2' * D2 and have one of the following structures:
...
where
      ...
      C = Diagonal(alpha(k+1), ... , alpha(k+l)),
      S = Diagonal(beta(k+1), ..., beta(k+l)),
      C' * C + S' * S = I
...
where
      ...
      C = Diagonal(alpha(k+1), ... , alpha(m)),
	  S = Diagonal(beta(k+1), ..., beta(m)),
      C' * C + S' * S = I
...
F: GSVD object.
"""
# gsvd() calls gsvd!() before making a copy of input matrices A and B,
