...
"""
safeDiag(Q1::AbstractMatrix{T}, Q2::AbstractMatrix{T}) where T = safeDiag!(copy(Q1),copy(Q2))

function safeDiag!(Q1::AbstractMatrix{T}, Q2::AbstractMatrix{T}) where T
    m, l = size(Q1)
	...
	# end
	#
	# # println(beta[r] <= 1/sqrt(2) && 1/sqrt(2) < beta[r+1])
	# # println("r is $r, l is $l, m-k is $m")
	# # println(min(m, l) > r)
	#
	# # Step 6: refine R
