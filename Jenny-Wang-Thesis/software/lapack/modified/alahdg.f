...
*     March 31, 1993
*
*     Modified by Jenny Wang (UCDavis) on July 2005
*       in order to add PSV and QSV in PATH     
*
*     .. Scalar Arguments ..
      CHARACTER*3       PATH
      INTEGER           IOUNIT
...
*             QSV:  Quotient/Generalized Singular Value Decomposition
...
*
      ...
      ELSE IF( LSAMEN( 3, C2, 'QSV' ) ) THEN
         ITYPE = 6
         WRITE( IOUNIT, FMT = 9996 )PATH
      ELSE IF( LSAMEN( 3, C2, 'PSV' ) ) THEN
         ITYPE = 7
...
*        PSVD
*
         WRITE( IOUNIT, FMT = 9945 )1
         WRITE( IOUNIT, FMT = 9946 )2
         WRITE( IOUNIT, FMT = 9947 )3
         WRITE( IOUNIT, FMT = 9948 )4
         WRITE( IOUNIT, FMT = 9949 )5
         WRITE( IOUNIT, FMT = 9149 )6
      ELSE IF( ITYPE.EQ.-1 ) THEN
*
*        CSD
*        
         WRITE( IOUNIT, FMT = 9199 )1
         WRITE( IOUNIT, FMT = 9198 )2
         WRITE( IOUNIT, FMT = 9197 )3
         WRITE( IOUNIT, FMT = 9196 )4
         WRITE( IOUNIT, FMT = 9195 )5
      END IF
*
 9999 FORMAT( 1X, A )
 ...
 9996 FORMAT( / 1X, A3, ': Quotient Singular Value Decomposition' )
 9997 FORMAT( / 1X, A3, ': Product Singular Value Decomposition' )
 9998 FORMAT( / 1X, A3, ': Cosine-Sine Decomposition' )
*
...
*
 ...
 9963 FORMAT( 3X, I2, ': Partioned Orthonormal Matrix')
*
*
*     GQR test ratio
*
...
*     PSVD test ratio
*
 9945 FORMAT( 3X, I2, ': norm( U'' * A * B * Q - R ) / ( min( M, N )*',
     $       'norm( A * B ) * EPS )' )
 9946 FORMAT( 3X, I2, ': norm( I - U''*U )   / ( M * EPS )' )
 9947 FORMAT( 3X, I2, ': norm( I - Q''*Q )   / ( N * EPS )' )
 9948 FORMAT( 3X, I2, ': norm( G - Q''diag(S) P'' ) / ( max( M, N ) *',
     $      'norm( G ) * EPS )' )
 9949 FORMAT( 3X, I2, ': norm( I - Q''*Q )   / ( M * EPS )' )      
 9149 FORMAT( 3X, I2, ': norm( I - P''*P )   / ( P * EPS )' )      
*
*     CSD test ratio
*
 9199 FORMAT( 3X, I2, ': norm( U'' * Q1 * Z - D1 ) / ( max( M, K )*',
     $      ' norm( Q1 ) * EPS )' )
 9198 FORMAT( 3X, I2, ': norm( V'' * Q2 * Z - D2 ) / ( max( P, K )*',
     $      ' norm( Q2 ) * EPS )' )
 9197 FORMAT( 3X, I2, ': norm( I - U'' * U ) / ( M * EPS )' )
 9196 FORMAT( 3X, I2, ': norm( I - V'' * V ) / ( P * EPS )' )
 9195 FORMAT( 3X, I2, ': norm( I - Z'' * Z ) / ( K * EPS )' )      
*      
      RETURN
*
*     End of ALAHDG
